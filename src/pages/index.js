...
const CTAButtons = () => (
	<div className="flex flex-col sm:flex-row w-full sm:w-auto">
		...
		<Reset className="block">
			<a
				href="https://github.com/tailwindlabs/awsicons"
				{...target_blank}
			>
				<div className="p-5 bg-orange-50 bg-opacity-18.75 dark:bg-opacity-12.5 rounded-3">
					<p className="text-center sm:text-left font-medium text-lg leading-none text-orange-50">
		...
		<Media className="block xl:hidden">
			<div className="py-6 absolute inset-x-0 bottom-full">
				<Style className="text-center">
					<p className="font-bold tracking-wide leading-none text-orange-50 opacity-87.5" style={{ fontSize: "0.6875rem" }}>
						<TextRow>
			...
			{css`
				@media (min-width: ${screens.lg}) {
					html {
						...
						background-image:
							url("data:image/svg+xml,%3Csvg fill='hsl(27, 87%, 50%)' viewBox='0 0 1 1' xmlns='http://www.w3.org/2000/svg'%3E%3Crect width='1' height='1' /%3E%3C/svg%3E"),
				...
				<div className="absolute left-0 inset-y-0 pointer-events-none">
					<div className="px-8 pr-4 flex flex-row h-full">
						<div className="flex flex-row items-center">
							<Style className="w-6 h-6 text-cool-gray-400 dark:text-cool-gray-600" style={{ color: inputFocused && "var(--orange-500)" }}>
							...
							<Reset className="focus:outline-none">
								<Style className="transition duration-200 ease-out">
									<button
										...
										style={{
											color: state.controls.theme.darkMode && "var(--orange-50)",
											backgroundColor: state.controls.theme.darkMode && "var(--orange-500)",
										}}
										onClick={e => (
											dispatch({
				...
				<div className="p-4 absolute inset-x-0 bottom-0">
					<div className="-mx-2 -my-1 flex flex-row justify-center">
						<Reset className="subpixel-antialiased">
							<p className="px-2 py-1 tracking-wide leading-tight text-cool-gray-600 dark:text-cool-gray-400 cursor-text select-text truncate" style={{ fontSize: px(13) }}>
								{!copyAsJSX ? (
									icon.name
								) : (
						...
						>
							<div id="app-grid">
								{state.search.__results.length < dataset.length && (
									<DocumentTitle title={`awsicons – ${state.search.__results.length} result${state.search.__results.length === 1 ? "" : "s"}`} />
								)}
